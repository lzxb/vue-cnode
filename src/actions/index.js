...
export default (...arg) => {
    ...
    const newPage = (name) => {
        ...
        actions.ADD_CUSTOM_KEY = ({dispatch}, ...arg) => { //添加自定义字段
            dispatch(`${name}ADD_CUSTOM_KEY`, ...arg)
        }

        actions.SET_CUSTOM_KEY = ({dispatch}, ...arg) => { //设置自定义字段
            dispatch(`${name}SET_CUSTOM_KEY`, ...arg)
        }
    }
